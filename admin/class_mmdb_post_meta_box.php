...
/**
 * Defines and orchestrates the post metabox and related admin side (search and 'selected') template rendering functionality
 *
...

{
	...
	public function mmdb_id_class_meta_box($post, $args) {
		...
		return $this->mmdb_get_meta_box_content($post_type, $mmdb_existing_id );
	...
	public function mmdb_get_meta_box_content($post_type, $mmdb_existing_id) {
	...
	public function mmdb_no_selection_template() {
		...
		$return = include plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/mmdb_no_selection.php';
	...
	public function my_ajax_search_mmdb() {
		...
		if (!empty($_POST['key'])) {
			...
			$result = $this->mmdb_search(sanitize_key($_POST['key']),sanitize_text_field($_POST['posttype']));
	 ...
	 * @param 	  string 	$post	 The post type returned from js
	 */
